...
func main() {
	...
	releases, err := fetchReleases("apangin", "jattach")
	if err != nil {
	...
	for _, release := range releases {
		...
		amd64Asset := findAsset(release.Assets, `^jattach-linux-x64\.tgz$`)
		arm64Asset := findAsset(release.Assets, `^jattach-linux-arm64\.tgz$`)
		...
		sourceURL := fmt.Sprintf("https://github.com/apangin/jattach/archive/refs/tags/%s.tar.gz", release.TagName)
		...
		cpe := fmt.Sprintf("cpe:2.3:a:apangin:jattach:%s:*:*:*:*:*:*:*", versionStr)
		purlAMD64 := fmt.Sprintf("pkg:generic/jattach@%s?arch=amd64", versionStr)
		purlARM64 := fmt.Sprintf("pkg:generic/jattach@%s?arch=arm64", versionStr)
